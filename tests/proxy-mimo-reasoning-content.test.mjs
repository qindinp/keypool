...
describe('fixMimoReasoningContent', () => {
  it('injects null only for assistant messages with tool_calls', () => {
    ...
    assert.equal('reasoning_content' in parsed.messages[1], false, 'summary-like assistant message should not be polluted');
    assert.equal(parsed.messages[2].reasoning_content, null, 'tool-call assistant should be patched');
    assert.equal(parsed.messages[3].reasoning_content, 'kept', 'existing reasoning_content should be preserved');
  });
  ...
  it('skips malformed message entries instead of failing the whole shim', () => {
    ...
    const parsed = JSON.parse(result.fixedBody);
    assert.equal(parsed.messages[3].reasoning_content, null);
  });
  ...
  it('does not patch non-MiMo models', () => {
    ...
    assert.equal(result.fixedBody, body);
  });
});
